...
from app.core.errors import OlympiadNotExists,InvalidDateConfiguration , OlympiadAlreadyActive
...
class OlympiadServices:
    def get_olympiad(self,olympiad_id:int,session:Session) -> Olympiad:
        olympiad = session.query(Olympiad).filter(Olympiad.id == olympiad_id,Olympiad.is_active == True).first()
        if olympiad is None:
    ...
    def create_olympiad(self,olympiad_data:OlympiadCreateModel,current_user:User,session:Session) -> Olympiad:
        try:
            
            
            new_olympiad=Olympiad(
                **olympiad_data.model_dump(),
    ...
    def update_olympiad(self,olympiad_id:int,olympiad_data:OlympiadUpdateModel,session:Session) -> Olympiad:
        try:
            ...
            registration_start=update_data.get("registration_start",olympiad.registration_start)
            registration_end=update_data.get("registration_end",olympiad.registration_end)
            exam_date=update_data.get("exam_date",olympiad.exam_date)
            
            if registration_start>=registration_end:
                raise InvalidDateConfiguration("Registration End date should be after registration start date")
            
            if registration_end>= exam_date:
                raise InvalidDateConfiguration("Exam date should be after registration end date")
