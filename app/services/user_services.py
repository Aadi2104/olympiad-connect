...
class UserServices:
    ...
    def initiate_signup(
        self, user_data: UserCreateModel, bg_tasks: BackgroundTasks, session: Session
    ) -> MessageResponseModel:
    ...
    def verify_user(
        self, otp_data: UserVerifyModel, token: str, session: Session
    ) -> User:
        try:
            ...
            new_user = User(
                email=signup_data["email"], password_hash=signup_data["password_hash"]
            )
    ...
    def validate_refresh_token(
        self,refresh_token: str, session: Session
    ) -> RefreshTokenModel:
        refresh_token_data = decode_refresh_token(refresh_token)
        refresh_token_record = (
            session.query(RefreshToken)
            .filter(RefreshToken.jti == refresh_token_data["jti"])
            .first()
        )
        ...
        if not verify_refresh_token(refresh_token, refresh_token_record.token_hash):
            raise InvalidToken("Refresh token is invalid")

        return refresh_token_record

    def login_user(
        self, user_data: UserLoginModel, session: Session
    ) -> UserLoginResponseModel:
        user_email = user_data.email
        user_password = user_data.password
    ...
    def logout_user(
        ...
        try:
            ...
            refresh_token_record = self.validate_refresh_token(refresh_token, session)
    ...
    def request_password_reset(
        self,
        email_data: UserForgotPasswordModel,
    ...
    def reset_password(
        self, password_data: UserResetPasswordModel, token: str, session: Session
    ) -> MessageResponseModel:
        try:
    ...
    def activate_user(
        self, user_data: UserManagementModel, session: Session
    ) -> MessageResponseModel:
        try:
